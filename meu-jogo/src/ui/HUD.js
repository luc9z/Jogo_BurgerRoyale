...
export default class HUD {
  ...
  _build() {
    ...
    const lbg = s.add.graphics().setDepth(D);
    lbg.fillStyle(COLOR.HUD_BG, 0.88);
    lbg.fillRoundedRect(lx, ly, lw, lh, 5);
    lbg.lineStyle(1, COLOR.HUD_BORDER, 1);
    lbg.strokeRoundedRect(lx, ly, lw, lh, 5);

    s.add.text(lx+8,  ly+6,  '❤', { fontSize:'12px', color:'#ff5555' }).setDepth(D+1);
    s.add.text(lx+24, ly+8,  'VIDA', ps('7px', '#ff5555')).setDepth(D+1);

    const hbx = lx+82, hby = ly+8, hbw = 168, hbh = 12;
    s.add.rectangle(hbx+hbw/2, hby+hbh/2, hbw, hbh, 0x330000).setDepth(D+1);
    this._hpBar  = s.add.rectangle(hbx, hby, hbw, hbh, COLOR.GREEN)
      .setOrigin(0,0).setDepth(D+2);
    this._hpText = s.add.text(hbx+hbw+5, hby, '100', ps('9px'))
      .setOrigin(0,0).setDepth(D+2);

    s.add.text(lx+8,  ly+30, '🔫', { fontSize:'10px' }).setDepth(D+1);
    s.add.text(lx+24, ly+32, 'AMMO', ps('7px', '#ffdd44')).setDepth(D+1);

    this._ammoIcons = [];
    for (let i = 0; i < PLAYER.CLIP_SIZE; i++) {
      const ic = s.add.rectangle(lx+82+i*16, ly+34, 11, 11, COLOR.GOLD_LIGHT)
        .setOrigin(0,0).setDepth(D+2);
      this._ammoIcons.push(ic);
    }
    this._reloadLbl = s.add.text(lx+82, ly+34, '', ps('8px', '#ff8800')).setDepth(D+2);

    // ── Painel direito (score + round) ────────────────────
    const rx = W-12-228, ry = 5, rw = 228, rh = 58;
    const rbg = s.add.graphics().setDepth(D);
    rbg.fillStyle(COLOR.HUD_BG, 0.88);
    rbg.fillRoundedRect(rx, ry, rw, rh, 5);
    rbg.lineStyle(1, COLOR.HUD_BORDER, 1);
    rbg.strokeRoundedRect(rx, ry, rw, rh, 5);

    s.add.text(rx+8, ry+8,  '⭐ PONTOS', ps('7px', '#ffd740')).setDepth(D+1);
    this._scoreTxt = s.add.text(rx+rw-8, ry+8, '0', ps('11px', '#ffd740'))
      .setOrigin(1, 0).setDepth(D+2);

    s.add.text(rx+8, ry+32, '⚔ ROUND',  ps('7px', '#ff8800')).setDepth(D+1);
    this._roundTxt = s.add.text(rx+rw-8, ry+32, '1', ps('11px', '#ff8800'))
      .setOrigin(1, 0).setDepth(D+2);

    // ── Inimigos restantes (centro-topo) ──────────────────
    ...
    // Hint
    s.add.text(W/2, H-5,
      'WASD mover  |  ESPAÇO atirar  |  R recarregar', {
        fontFamily: '"Press Start 2P", monospace',
  ...
  _showBanner(round, count) {
    ...
    this._bannerSub.setText(`${count} palhaços se aproximando...`);
  ...
  showRoundClear(round) {
    ...
    this._bannerSub.setText('Próxima horda chegando...');
    this.scene.tweens.add({ targets:[this._banner, this._bannerSub], alpha:1, duration:350,
      onComplete: () => {
