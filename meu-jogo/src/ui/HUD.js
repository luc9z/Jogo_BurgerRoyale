...
export default class HUD {
  ...
  _build() {
    ...
    s.add.text(lx+8,  ly+7,  '❤', { fontSize:'12px', color:'#ff5555' }).setDepth(D+1);
    s.add.text(lx+24, ly+8,  'VIDA', ps('7px','#ff5555')).setDepth(D+1);
    const hbx=lx+80, hby=ly+8, hbw=170, hbh=12;
    ...
    for (let i = 0; i < PLAYER.CLIP_SIZE; i++) {
      const ic = s.add.rectangle(lx+80+i*16, ly+34, 11, 11, COLOR.GOLD_LIGHT).setOrigin(0,0).setDepth(D+2);
      this._ammoIcons.push(ic);
    }
    this._reloadLbl = s.add.text(lx+80, ly+34, '', ps('8px','#ff8800')).setDepth(D+2);
    ...
    this._enemyTxt = s.add.text(W/2, 12, '', {
      fontFamily:'"Press Start 2P", monospace', fontSize:'8px', color:'#ff5555',
      stroke:'#000', strokeThickness:3,
    }).setOrigin(0.5,0).setDepth(D+1);

    // Banners
    this._banner = s.add.text(W/2, H/2-38, '', {
      fontFamily:'"Press Start 2P", monospace', fontSize:'26px', color:'#ffd740',
      stroke:'#000000', strokeThickness:8,
    }).setOrigin(0.5).setDepth(DEPTH.BANNER).setAlpha(0);
    this._bannerSub = s.add.text(W/2, H/2+12, '', {
      fontFamily:'"Press Start 2P", monospace', fontSize:'9px', color:'#ff8800',
      stroke:'#000', strokeThickness:4,
    }).setOrigin(0.5).setDepth(DEPTH.BANNER).setAlpha(0);

    s.add.text(W/2, H-5, 'WASD mover  |  ESPAÇO atirar  |  R recarregar', {
      fontFamily:'"Press Start 2P", monospace', fontSize:'6px', color:'#ffffff22',
